...
Eks::Transform GCVisualManipulator::resultTransform(const RCViewableTransform *tr) const
  {
...
Eks::Vector3D GCVisualManipulator::focalPoint(const RCViewableTransform *cam) const
  {
  if(delegate())
    {
...
void GCVisualCompoundManipulator::render(const RCCamera *camera, Eks::Renderer *r) const
  {
  xForeach(auto m, walker<GCVisualManipulator>())
    {
    m->render(camera, r);
...
void GCAngularDragManipulator::onDrag(const MouseMoveEvent &e, Eks::Quaternion &rel)
  ...
  if(lock == Planar)
    ...
    auto isectA = a.sample(p.intersection(a));
    auto isectB = b.sample(p.intersection(b));
    
    auto subA = isectA - focus;
    auto subB = isectB - focus;
